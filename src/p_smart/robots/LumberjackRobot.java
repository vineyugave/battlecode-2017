package p_smart.robots;

import battlecode.common.*;
import ddframework.algorithms.AStar;
import ddframework.managers.MovementManager;
...
/**
 * Created by Viney Ugave (viney@vinzzz.com) on 1/24/17
 */
...
public class LumberjackRobot extends SmartBaseRobot {
    ...
    public static final float STRIDE_LENGHT = 0.75f;

    public LumberjackRobot(RobotController controller) {
        super(controller);
        movementManager = new MovementManager(controller);
    }
    ...
    protected void onGameRound(RobotController rc) throws Exception {
        ...
        if (robots.length > 0 && !rc.hasAttacked()) {
            ...
            if (robots.length > 0) {
                ...
                //figure out path to enemy
                AStar pathFinder = (AStar) movementManager.getAlgorithmStrategy(MovementManager.Algorithms.AStar);
                pathFinder.setCosts(STRIDE_LENGHT,STRIDE_LENGHT); // setting cost same for now
                pathFinder.setStartCell(pathFinder.roundLocation(myLocation.x), pathFinder.roundLocation(myLocation.y));
                pathFinder.setEndCell(pathFinder.roundLocation(enemyLocation.x), pathFinder.roundLocation(enemyLocation.y));
                //TODO need to dynamically set the map grid /resolution
